...
class agent:
    def __init__(self, game):
        ...
        self.train_file = os.path.join(class_dir, 'DG.train')
        ...
        # print(sum(sum(self.dg.dp[i]) for i in range(len(self.dg.dp))))
    ...
    def train(self):
        edge0 = [[] for _ in range(self.dg.n)]
        edge1 = [[] for _ in range(self.dg.n)]
        win = set()
        lose = set()
        for msk in range(self.dg.n):
            self.game.reset()
        ...
        self.dg.set_edges(edge0, edge1)
        self.dg.set_win(win)
        self.dg.set_lose(lose)
        self.dg.solve()
    ...
    def make_move(self):
        ...
        for t in range(9):
            ...
            if p == 1:
                ...
                moves.append([t, self.dg.dp[msk_][1], self.dg.depth[msk_][1]])
            else:
                ...
                moves.append([t, -self.dg.dp[msk_][0], self.dg.depth[msk_][0]])
