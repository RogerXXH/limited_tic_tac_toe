...
- ✅ 标准型数量已计算：**72,864,169**
- ✅ 存储方案已优化（mmap + 二分查找）
- ✅ C++训练器已实现（`train_4x4_m4.cpp`）
- ❌ C++训练器运行到 26.2% 时被 OOM Kill，WSL 崩溃
- ✅ OOM 根因已分析，新方案已设计
- 🔄 下一步：重写训练器（不存储边，BFS 时动态推导反向邻居）
...
**已完成**：
- `train_4x4_m4.cpp` 完整实现（Python 逻辑 1:1 移植到 C++）
- 编译：`g++ -O2 -std=c++17 -o train_4x4_m4 train_4x4_m4.cpp`
- 首次运行到 **26.2%** 时，进程被 OOM Kill，WSL 显示 `Wsl/Service/E_UNEXPECTED` 崩溃

**系统内存**：
- WSL 总内存：**7.7 GB RAM + 2 GB Swap = 9.7 GB**

---

### 内存超限根因分析

C++ 版本与 Python 版本结构相同，问题也相同：**把所有边存进 `unordered_map<uint64_t, vector<uint64_t>>`**。

内存估算（72M 状态时）：

| 数据结构 | 估算内存 |
|---------|---------|
| `g_states`（StateInfo × 72M） | ~2-3 GB |
| `g_edge0` + `g_edge1`（~10条边/状态 × 8字节 + map开销） | ~10-15 GB |
| `canons` set | ~2-3 GB |
| 合计 | **~15-20 GB** |

即使 C++ 比 Python 节省 3-5 倍内存，在 9.7 GB 的机器上也远远不够。
在 26.2% 时崩溃，是因为 `add_edge` 会同时把当前状态的**所有邻居**加入 solver，邻居遍布全部 72M 状态空间，导致内存快速膨胀。

---

### 新方案：不存储边，BFS 时动态计算反向邻居

**核心思路**：
- 传播阶段不需要预存边，只需要知道"谁指向当前状态"
- 给定标准型 (x, y)，其**前驱状态**可以通过反向推导实时计算：
  - 如果 `len(x) < MAX_MOVE`：前驱 x 去掉最后一个棋子
  - 如果 `len(x) == MAX_MOVE`：前驱 x 在最前面有一个已消失的棋子（枚举所有可能位置）
- 这样彻底消除边的存储，节省 10-15 GB

**新方案内存估算**（72M 状态）：

| 数据结构 | 内存 |
|---------|------|
| 所有 state code 的排序数组 | 72M × 8B = 576 MB |
| dp 数组（索引寻址） | 72M × 2B = 144 MB |
| depth 数组 | 72M × 4B = 288 MB |
| need 数组（出度计数） | 72M × 8B = 576 MB |
| BFS 队列 | 最多 72M × 4B = 288 MB |
| **合计** | **~1.9 GB** ✅ |

枚举阶段只需要一个 `unordered_set<uint64_t>` 存所有标准型 code（~2-3 GB），枚举完成后排序转为数组，释放 hash set，进入 BFS 阶段。

---

### win/lose 状态精确计数

新增工具 `count_win_lose.cpp`，统计 win/lose 标准型数量。
...
| 文件 | 说明 |
|------|------|
| `train_4x4_m4.cpp` | C++ 训练器（旧方案，存储边，OOM） |
| `train_4x4_m4` | 编译后的可执行文件 |
| `test_storage.cpp` / `test_storage` | 存储格式验证工具 |
| `verify_storage.py` | Python 端存储验证 |
| `count_win_lose.cpp` / `count_win_lose` | win/lose 标准型计数工具 ✅ |
...
1. **枚举阶段**（Pass 1）
   - 枚举所有标准型，收集 code 到 `unordered_set`
   - 同时统计每个状态的出度（不存边，只存计数）
   - 标记 win/lose 状态
   - 完成后排序转为 `vector<uint64_t>`，建立 code→index 映射（二分查找）

2. **BFS 传播阶段**（Pass 2）
   - 从 win/lose 状态出发，BFS
   - 每次弹出一个状态，**动态计算其所有前驱**（反向邻居推导）
   - 更新前驱的 need 计数，need 归零则加入队列
